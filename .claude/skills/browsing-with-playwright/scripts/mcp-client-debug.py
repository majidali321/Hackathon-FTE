...
class HTTPTransport:
    ...
    def _parse_response(self, body: str) -> dict:
        ...
        body = body.strip()
        print(f"DEBUG _parse_response: body length = {len(body)}", file=sys.stderr)
        print(f"DEBUG _parse_response: body[:200] = {repr(body[:200])}", file=sys.stderr)
    ...
    def request(self, method: str, params: Optional[dict] = None) -> dict:
        ...
        print(f"DEBUG request: method={method}, params={params}", file=sys.stderr)
        data = json.dumps(payload).encode('utf-8')
        print(f"DEBUG request: payload={payload}", file=sys.stderr)
        headers = {
            "Content-Type": "application/json",
            "Accept": "application/json, text/event-stream",
            **self.headers
        ...
        if self._session_id:
            headers["Mcp-Session-Id"] = self._session_id
            print(f"DEBUG request: session_id={self._session_id}", file=sys.stderr)
        ...
        try:
            with urlopen(req, timeout=30) as resp:
                ...
                print(f"DEBUG request: response body length={len(body)}", file=sys.stderr)
...
def main():
    ...
    try:
        ...
        if args.command == "list":
            ...
            print(f"DEBUG: args.params = '{args.params}'", file=sys.stderr)
            print(f"DEBUG: type = {type(args.params)}", file=sys.stderr)
            print(f"DEBUG: repr = {repr(args.params)}", file=sys.stderr)
            params = json.loads(args.params)
            print(f"DEBUG: Parsed params = {params}", file=sys.stderr)
            print(f"DEBUG: Calling tool {args.tool}", file=sys.stderr)
